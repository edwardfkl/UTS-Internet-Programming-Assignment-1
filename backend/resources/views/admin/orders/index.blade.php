...
@php
    $statusBadge = [
        \App\Models\Order::STATUS_CART => 'bg-zinc-100 text-zinc-700',
        \App\Models\Order::STATUS_PENDING_PAYMENT => 'bg-amber-100 text-amber-900',
        \App\Models\Order::STATUS_PAID => 'bg-sky-100 text-sky-900',
        \App\Models\Order::STATUS_SHIPPED => 'bg-indigo-100 text-indigo-900',
        \App\Models\Order::STATUS_COMPLETED => 'bg-emerald-100 text-emerald-900',
        \App\Models\Order::STATUS_CANCELLED => 'bg-red-100 text-red-900',
    ];
@endphp
...
@section('content')
    ...
    <form method="post" action="{{ route('admin.orders.bulk') }}" data-bulk-form class="mt-4">
        @csrf
        ...
        <div class="overflow-x-auto rounded-xl border border-zinc-200 bg-white shadow-sm">
            <table class="min-w-full divide-y divide-zinc-200 text-sm">
                ...
                @forelse ($orders as $order)
                    <tr class="hover:bg-zinc-50/80">
                        <td class="px-3 py-3 text-center">
                            <input type="checkbox" name="ids[]" value="{{ $order->id }}" data-bulk-id
                                   class="rounded border-zinc-300 text-amber-900">
                        </td>
                        ...
                        <td class="whitespace-nowrap px-4 py-3">
                            <span class="rounded-full px-2 py-0.5 text-xs font-medium {{ $statusBadge[$order->status] ?? 'bg-zinc-100 text-zinc-800' }}">
                                {{ \Illuminate\Support\Facades\Lang::has('admin.orders.status_labels.'.$order->status) ? __('admin.orders.status_labels.'.$order->status) : $order->status }}
                            </span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="10" class="px-4 py-8 text-center text-zinc-500">{{ __('admin.orders.empty') }}</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
        ...
        @include('admin.partials.bulk-toolbar', [
            'bulkRoute' => 'admin.orders.bulk',
            'bulkStatuses' => $statuses,
            'bulkResourceKey' => 'admin.orders',
        ])
    </form>
